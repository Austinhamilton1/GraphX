 ...
 *     int32_t u - Source node.
 *     int32_t v - Destination node.
 * Returns:
 *     int32_t - The weight between u and v.
 */
int32_t graph_get_weight(graph_t *graph, int32_t u, int32_t v) {
    // Start and end of the row
    int32_t start = graph->row_index[u];
    int32_t end = graph->row_index[u+1];

    for(size_t i = start; i < end; i++) {
        if(graph->col_index[i] == v)
            return graph->values[i];
    }
 ...
 * Arguments:
 *     graph_t *graph - The graph to query.
 *     int32_t - Query this node.
 *     int *size - Number of neighbors will go here.
 * Returns:
 *     int32_t * - A pointer to the adjacency list of u.
 */
 ...
 *     int32_t *neighbor - Save the neighbor here (set to NULL if unwanted).
 *     int32_t *weight - Save the weight here (set to NULL if unwanted).
 * Returns:
 ...
 * Arguments:
 *     graph_t *graph - Graph to query.
 *     int32_t node - Query this node.
 * Returns:
 *     size_t - The degree of node.
 */
size_t graph_degree(graph_t *graph, int32_t node) {
    // Start and end of the row
    int32_t start = graph->row_index[node];
    int32_t end = graph->row_index[node+1];
